...
namespace SDDM {
        ...
        do {
            ...
            switch (m) {
                case ERROR: {
                    int result;
                    QString message;
                    Auth::Error type = Auth::ERROR_NONE;
                ...
                default: {
                    ...
                    Q_EMIT auth->error(QStringLiteral("Auth: Unexpected value received: %1").arg(m), Auth::ERROR_INTERNAL, 0);
                }
            }
        } while(str.available());
    }
    ...
    void Auth::Private::childExited(int exitCode, QProcess::ExitStatus exitStatus) {
        if (exitStatus != QProcess::NormalExit) {
            qWarning("Auth: sddm-helper (%s) crashed (exit status %d, exit code %d)",
                     ...
                     Auth::HelperExitStatus(exitStatus), exitCode);
            Q_EMIT qobject_cast<Auth*>(parent())->error(child->errorString(), Auth::ERROR_INTERNAL, 0);
        }
    ...
    void Auth::Private::childError(QProcess::ProcessError error) {
        Q_UNUSED(error);
        Q_EMIT qobject_cast<Auth*>(parent())->error(child->errorString(), ERROR_INTERNAL, 0);
    }
