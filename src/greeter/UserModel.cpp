...
namespace SDDM {
    ...
    UserModel::UserModel(QObject *parent) : QAbstractListModel(parent), d(new UserModelPrivate()) {
        ...
        if (avatarsEnabled && mainConfig.Theme.EnableAvatars.isDefault()) {
            if (d->users.count() > mainConfig.Theme.UsersThresholdToDisableAvatars.get()) avatarsEnabled=false;
        }
    ...
    int UserModel::usersThresholdToDisableAvatars() const {
        return mainConfig.Theme.UsersThresholdToDisableAvatars.get();
    }
}
