...
namespace SDDM {
    ...
    */
    ...
    PamBackend::PamBackend(HelperApp *parent)
        ...
        QObject::connect(m_pam, SIGNAL(error(QString, Auth::Error, int)),
                         parent, SLOT(error(QString, Auth::Error, int)));
    }
    ...
    bool PamBackend::start(const QString &user) {
        ...
        if (!result)
            m_app->error(m_pam->errorString(), Auth::ERROR_INTERNAL, m_pam->getResult());
    ...
    bool PamBackend::authenticate() {
        ...
        if (!m_pam->authenticate()) {
            m_app->error(m_pam->errorString(), Auth::ERROR_AUTHENTICATION, m_pam->getResult());
            return false;
        }
        if (!m_pam->acctMgmt()) {
            m_app->error(m_pam->errorString(), Auth::ERROR_AUTHENTICATION, m_pam->getResult());
    ...
    int PamBackend::converse(int n, const struct pam_message **msg, struct pam_response **resp) {
        ...
        qDebug().noquote().nospace() << PAM_LOG_PREFIX << "pam_conv" << ": state = " << m_workState;
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch(msg[i]->msg_style) {
                ...
                case PAM_ERROR_MSG:
                    ...
                    m_app->error(convMsg, Auth::ERROR_PAM_CONV, m_pam->getResult());
                    break;
                case PAM_TEXT_INFO:
                    ...
                    m_app->info(convMsg, Auth::INFO_PAM_CONV, m_pam->getResult());
                    break;
                default:
                    break;
            }
