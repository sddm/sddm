...
namespace SDDM {
    ...
    void Display::slotRequestChanged() {
        Request *currentRequest = m_auth->request();

        if (currentRequest->prompts().length() == 1) {
            currentRequest->prompts()[0]->setResponse(qPrintable(m_passPhrase));
            currentRequest->done();
        } else if (currentRequest->prompts().length() == 2) {
            currentRequest->prompts()[0]->setResponse(qPrintable(m_auth->user()));
            currentRequest->prompts()[1]->setResponse(qPrintable(m_passPhrase));
            currentRequest->done();
        }
    }
